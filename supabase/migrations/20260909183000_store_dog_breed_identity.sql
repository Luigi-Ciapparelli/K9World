-- Store FCI breed identity for the first dog created during signup.
...
declare
  ...
  dog_fci_group_value integer := null;
begin
  ...
  if signup_role = 'professional' then
    ...
    if coalesce(meta ->> 'dog_fci_group', '') ~ '^(10|[1-9])$' then
      dog_fci_group_value := (meta ->> 'dog_fci_group')::integer;
    end if;
    ...
    values (
      ...
      nullif(trim(coalesce(meta ->> 'dog_breed_slug', '')), ''),
      dog_fci_group_value,
      dog_age_value,
      dog_weight_value,
      '',
